...
/**
 * Try direct key, then **whole-word** containment match. The earlier
 * bidirectional substring match returned false positives — "India"
 * matched "Mumbai Indians" because "mumbai indians".includes("india").
 * Whole-word match means tokens have to actually appear, e.g.
 * "manchester united" → matches key "manchester united" or "man united".
 */
function lookup(table: Record<string, string>, name: string): string | undefined {
  ...
  for (const k of Object.keys(table)) {
    ...
    // The entry is a match when every token of one side appears in the
    // other. "bayern" matches "fc bayern münchen" → keyTokens=['bayern']
    // all appear in queryTokens=['bayern','munchen','fc']. ✓
    // "india" against "mumbai indians" → keyTokens=['mumbai','indians']
    // does NOT all appear in queryTokens=['india']. ✗ (no false positive)
    const allKeyIn = keyTokens.every((t) => queryTokens.includes(t));
    const allQueryIn = queryTokens.every((t) => keyTokens.includes(t));
    if (allKeyIn || allQueryIn) return table[k];
  }
  return undefined;
}
...
const CRICKET: Record<string, string> = {
  ...
  'delhi capitals': 'https://upload.wikimedia.org/wikipedia/en/4/4d/Delhi_Capitals.svg',
